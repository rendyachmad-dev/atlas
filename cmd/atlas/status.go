...
func runStatus() {
	...
	if len(os.Args) > 2 {
		...
		case "help", "-h", "--help":
			printStatusHelp()
		default:
			fmt.Fprintf(os.Stderr, "unknown status subcommand: %s\n\n", os.Args[2])
			printStatusHelp()
			os.Exit(1)
		}
		return
...
func printStatusHelp() {
	fmt.Println("Usage: atlas status [subcommand]")
	fmt.Println()
...
func printDashboard(ctx context.Context, pool *pgxpool.Pool) {
	...
	fmt.Println("Subcommands: atlas status sources | logs [N] | reports [N]")
}
