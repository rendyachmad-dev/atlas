...
// Service orchestrates the intelligence pipeline.
type Service struct {
	store    *JobStore
	worker   *Worker
	pool     *pgxpool.Pool
...
func NewService(pool *pgxpool.Pool, provider IntelligenceProvider) *Service {
...
func (s *Service) Enqueue(ctx context.Context, priority int) (int, error) {
	...
	log.Printf("intelligence: enqueued %d articles", count)
	return count, nil
}
...
func (s *Service) RunDaemon(ctx context.Context, interval time.Duration) {
	log.Printf("intelligence: daemon started — poll interval %s", interval)
	...
	for {
		select {
		case <-ctx.Done():
			log.Println("intelligence: daemon shutting down")
			return
		case <-ticker.C:
			processed, _ := s.RunAll(ctx, 0)
			if processed > 0 {
				log.Printf("intelligence: daemon processed %d jobs", processed)
			}
		}
	}
}